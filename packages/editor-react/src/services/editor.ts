...
class Editor extends BaseService {
  ...
  public async selectNextNode(): Promise<MNode | null> | never {
    ...
    const parent = this.getParentById(node.id);
  ...
  public async doAdd(node: MNode, parent: MContainer): Promise<MNode> {
    ...
    if (
      (parent.type === NodeType.ROOT || curNode?.type === NodeType.ROOT) &&
    ...
    const layout = await this.getLayout(parent, node as MNode);
  ...
  public async add(
    ...
    if (!Array.isArray(addNode)) {
      ...
      addNodes.push({ ...(await propsService.getPropsValue(type, config)) });
    } else {
  ...
  public async doUpdate(config: MNode) {
    ...
    const node = cloneDeep(info.node);

    let newConfig = await this.toggleFixedPosition(config, node, root);
    ...
    const targetIndex = nodes.findIndex(
      (nodeItem: MNode) => `${nodeItem.id}` === `${newConfig.id}`
    );
  ...
  public async sort(id1: Id, id2: Id): Promise<void> {
    ...
    const parent = cloneDeep(this.get("parent"));
  ...
  public async move(left: number, top: number) {
    ...
    const update = (style: { [key: string]: unknown }) =>
      this.update({
        id,
        type,
        style,
  ...
  private pushHistoryState() {
    const curNode = cloneDeep(this.get("node"));
    const page = this.get("page");
