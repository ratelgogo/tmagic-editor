...
import { generatePageNameByApp, getInitPositionStyle } from './editor';
...
export const getDefaultConfig = async (addNode: AddMNode, parentNode: MContainer) => {
  ...
  const layout = await editorService.getLayout(parentNode, addNode as MNode);
  const newNode = { ...(await propsService.getPropsValue(type, config)) };
